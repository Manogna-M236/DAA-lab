...
{ 
    int dist[V], visited[V]; // Visited array acts as a "queue" to track which vertices are still to be processed 
    ...
    */ 
    ...
    { 
        int u = extractMin(dist, visited); /* Indentify & Dequeue the vertex u with the minimum distance */ 
        if (u == -1) 
            break; 
